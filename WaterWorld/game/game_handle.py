import numpy as np
import pygame
from WaterWorld.game.waterworld_handle import WaterWorldHandle


class WaterWorldGame:
    def __init__(self, width: int, height: int, num_creeps: int):
        pygame.init()
        self.current_game = WaterWorldHandle(width=width, height=height, num_creeps=num_creeps, enable_sensors=True)
